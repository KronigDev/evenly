...
export default function EditExpensePage() {
  ...
  const expenseId = readParam(params.expenseId);
  ...
  const expenseQuery = useQuery({
    ...
    queryFn: () => apiFetch<ExpenseDTO>(`/api/expenses/${expenseId}`),
    enabled: expenseId.length > 0,
  });

  const loading = groupQuery.isLoading || expenseQuery.isLoading;
  const failed = groupQuery.isError || expenseQuery.isError;
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-6 sm:py-8">
      ...
      {loading ? (
        ...
        <ExpenseForm group={group} members={group.members} mode="edit" expense={expense} />
      )}
    </div>
  );
}
